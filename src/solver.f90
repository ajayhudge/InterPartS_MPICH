...
contains
  subroutine solver2d(pz)
    ...
    real, dimension(itot) :: px_temp
    real, dimension(jtot) :: py_temp
    real :: bb
    ...
    !$omp&firstprivate(wi,px_temp)
    !$omp do
    do k=1,xsize(3)
       do j=1,xsize(2)
          px_temp = px(1:itot,j,k)
          call vrfftf(1,itot,px_temp,di,1,wi)
          px(1:itot,j,k) = px_temp
       enddo
    enddo
    !$omp end parallel
    !
    ...
    !$omp&firstprivate(wj,py_temp)
    !$omp do
    do k=1,ysize(3)
       do i=1,ysize(1)
          py_temp = py(i,1:jtot,k)
          call vrfftf(1,jtot,py_temp,dj,1,wj)
          py(i,1:jtot,k) = py_temp
       enddo
    enddo
    !$omp end parallel
    !
    ...
    do k=1,ysize(3)
       do i=1,ysize(1)
          py_temp = py(i,1:jtot,k)
          call vrfftb(1,jtot,py_temp,dj,1,wj)
          py(i,1:jtot,k) = py_temp
       enddo
    enddo
    !$omp end parallel
    ...
    do k=1,xsize(3)
       do j=1,xsize(2)
          px_temp = px(1:itot,j,k)
          call vrfftb(1,itot,px_temp,di,1,wi)
          px(1:itot,j,k) = px_temp
       enddo
    enddo
    !$omp end parallel
