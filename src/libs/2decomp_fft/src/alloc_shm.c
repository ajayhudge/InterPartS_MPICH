                  ...
                  MPI_Fint *ncores, MPI_Fint *my_core, MPI_Fint *maxcor)
{
    ...
    world = MPI_Comm_f2c(*comm); // mpi_bcast(world) at line 200, this communicator is obtained by MPI_Comm_f2c
    // to allow Fortran to flexibly pass different communicators to the C side. Replacing world
    // with MPI_COMM_WORLD may lead to incorrection (SS).
