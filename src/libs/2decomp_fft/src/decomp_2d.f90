...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine decomp_2d_init(nx,ny,nz,p_row,p_col,periodic_bc)
    ...
    logical, dimension(2) :: conserve_dims 
    ...
    conserve_dims = (/.true.,.false./)
    call MPI_CART_SUB(DECOMP_2D_COMM_CART_X,conserve_dims, &
         DECOMP_2D_COMM_COL,ierror)
    conserve_dims = (/.false.,.true./)
    call MPI_CART_SUB(DECOMP_2D_COMM_CART_X,conserve_dims, &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine prepare_buffer(decomp)
    ...
    integer :: i
    integer :: k
    integer, dimension(2) :: partposition
    ...
    do k=0,dims(1)-1
       ...
       do i=0,dims(2)-1
          ...
          call MPI_Cart_rank(DECOMP_2D_COMM_CART_X,partposition,rank_x,ierror)
          ! this call fails, since DECOMP_2D_COMM_CART_Z is not yet created
          !           call MPI_Cart_rank(DECOMP_2D_COMM_CART_Z,(/k,i/),rank_z,ierror)
          rank_z=rank_x
          !JD no checks on x- or z-dimension, since we transform from z- into x-pencils,
          !so these always overlap.
          if (decomp%zst(2).le.decomp%y1en(k) .and. &
             ...
             array_of_subsizes = (/decomp%zsz(1),subsize_y,decomp%z2dist(i)/) 
             array_of_starts = (/0,offset_y,decomp%z2st(i)-decomp%zst(3)/)
          ...
          if (decomp%xst(2).le.decomp%y2en(i) .and. &
             ...
             array_of_subsizes = (/decomp%x1dist(k),subsize_y,decomp%xsz(3)/) 
             array_of_starts = (/decomp%x1st(k)-decomp%xst(1),offset_y,0/)
