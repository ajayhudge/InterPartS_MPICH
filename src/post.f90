...
contains
  ...
  subroutine vorticity(ux,uy,uz,vox,voy,voz)
    ...
    !$omp&shared(ux,uy,uz,vox,voy,voz) &
    !$omp&private(i,j,k)
    !$omp do
    do k=1,kmax
       do j=1,jmax
  ...
  subroutine compute_r(ux,uy,uz,rr)
    ...
    !
    !$omp parallel default(none) &
    !$omp&shared(ux,uy,uz,rr) &
    !$omp&private(i,j,k,s11,s12,s13,s22,s23,s33,s21,s31,s32,vox,voy,voz)
    !$omp do
    do k=1,kmax
       do j=1,jmax
          do i=1,imax
