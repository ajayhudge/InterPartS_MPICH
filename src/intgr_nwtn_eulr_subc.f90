module mod_intgr_nwtn_eulr_subc
  use mpi
  use mod_common
  use mod_common_mpi
  ...
  implicit none
  private
  public intgr_nwtn_eulr_subc
contains
  !
  subroutine intgr_nwtn_eulr_subc(isubstep,iend,dtsub)
    implicit none
    integer, intent(in) :: isubstep,iend
...
!    real :: isperiodx,isperiody
    ...
    !$omp&shared(ap,op,pmax,isubstep) &
    !$omp&private(p)
    !$omp do 
    do p=1,pmax
    ...
    do while (iter.lt.itermax)!.and.(sumcolrank_all+pmax*Nproc).ne.0.and.maxerror*dxi.gt.1.e-5
       ...
       do nb=1,8
          ...
          nbrec  = nb
          if (nbsend .gt. 8) nbsend = nbsend - 8
          call MPI_SENDRECV(anb(1,0)%x,pmax*11,MPI_REAL8,neighbor(nbsend),3, &
                ...
                do nb=0,8
                   do q=1,pmax_nb(nb)
                      ...
                      elseif(mslv_nb(q,nb).gt.0) then
                         idq = mslv_nb(q,nb)
                         if((abs(ap(p)%x-anb(q,nb)%x).lt.4.*radius) .and. &
                              (abs(ap(p)%y-anb(q,nb)%y).lt.4.*radius) .and. &
                              (abs(ap(p)%z-anb(q,nb)%z).lt.4.*radius) ) then
                            count_friends = count_friends + 1
                            ap(p)%friend(count_friends) = idq
                         endif
                      endif
                   enddo
                enddo
                do idq=botw,topw
                   deltaz = (idq-botw)*lz - ap(p)%z
                   if(abs(deltaz).lt.2.*radius) then
                      count_friends = count_friends + 1
                      ap(p)%friend(count_friends) = idq
                   endif
                enddo
             endif
             ap(p)%nfriends = count_friends
          enddo
       endif
       !
       ! collision model
       ...
       do p=1,pmax
          ...
          if(ap(p)%mslv.gt.0) then
             ...
             do nb=0,8
                do q=1,pmax_nb(nb)
                   if((nb.ge.0).and.(idp.eq.mslv_nb(q,nb))) then
                      ...
                      !        elseif(mslv_nb(q,nb).gt.0.and.any(mslv_nb(q,nb).eq.ap(p)%friend(1:count_friends))) then
                   elseif(mslv_nb(q,nb).gt.0) then
                      idq = mslv_nb(q,nb)
                      ...
                      if((eps.lt.eps_ini_pp).and.(eps.gt.eps_cut_pp)) then
                         ...
                         !            if(iter.eq.0) print*,'lubrication_pp',idp,idq
                      endif
                      qq = 0
                      !
                         ...
                         if(idq.eq.ap(p)%firstc(i)) then
                            ...
                            if(dist.lt.(2.*radius)) then 
                               ...
                               call collisions(p,q,qq,idp,idq,dist,deltax,deltay,deltaz, &
                                    anb(q,nb)%u,anb(q,nb)%v,anb(q,nb)%w, &
                                    anb(q,nb)%omx,anb(q,nb)%omy,anb(q,nb)%omz,dtsub)
                               if ((deltan*nx).lt.(2.*radius-colthr_pp)) colflgx(p) = 1. ! IBM force off
                               if ((deltan*ny).lt.(2.*radius-colthr_pp)) colflgy(p) = 1. ! IBM force off
                               if ((deltan*nz).lt.(2.*radius-colthr_pp)) colflgz(p) = 1. ! IBM force off
                               colrank(p) = myid
                               if(iter.eq.-1) print*,'collision_cont',idp,idq,iter
                            elseif(iter.eq.itermax) then
                               ...
                               op(p)%dwt(qlast) = 0.
                               qlast = qlast - 1
                               if(iter.eq.-1) print*,'collision_last',idp,idq,iter
                            endif
                         endif
                      enddo
                      if(qq.eq.0.and.dist.lt.(2.*radius)) then 
                         ...
                         colrank(p) = myid
                         if(iter.eq.-1) print*,'collision_1st',idp,idq,iter
                      endif
             ...
             do q=botw,topw
                idq = q
                !      if(any(idq.eq.ap(p)%friend(1:count_friends))) then
                deltax = 0.
                deltay = 0.
                ...
                if((eps.lt.eps_ini_pw).and.(eps.gt.eps_cut_pw)) then
                   ...
                   call lubrication(p,q,idp,idq,nx,ny,nz,eps, &
                        0.,0.,0.,0.,0.,0.)
                   !        if(iter.eq.0) print*,'lubrication_pw',idp,idq,eps
                endif
                do i=1,ap(p)%qmax
                   if(idq.eq.ap(p)%firstc(i)) then
                      !
                      ...
                      if(dist.lt.radius) then
                         !            if(iter.eq.iter) print*, 'collision, pw',idp,idq
                         call collisions(p,q,qq,idp,idq,dist,deltax,deltay,deltaz, &
                              0.,0.,0.,0.,0.,0.,dtsub)
                         ...
                         !            if(iter.eq.-1) print*,'collision_cont',idp,idq,iter
                      elseif(iter.eq.itermax) then
                         !            if(iter.eq.iter) print*, 'collision, ended',idp,idq,dist
                         if(qlast.gt.1) then
                            ap(p)%firstc(i) = ap(p)%firstc(qlast)
                            ap(p)%dxt(i) = ap(p)%dxt(qlast)
                            ap(p)%dyt(i) = ap(p)%dyt(qlast)
                         ...
                         op(p)%dwt(qlast) = 0.
                         qlast = qlast - 1
                         !            if(iter.eq.-1) print*,'collision_end',idp,idq
                      endif
                   endif
                enddo
                if((qq.eq.0).and.(dist.lt.radius)) then
                   !        if(iter.eq.iter) print*, 'collision, pw 1st',idp,idq,dist
                   qlast = qlast + 1
                   qq = qlast
                   ap(p)%dx(qlast) = 0.
                   ap(p)%dy(qlast) = 0.
                   ...
                   ap(p)%firstc(qlast) = idq
                   op(p)%firstc(qlast) = idq
                   !        if(iter.eq.-1) print*,'collision_1st',idp,idq
                endif
                !      endif
             enddo
             ap(p)%qmax = qlast
...
!!$omp do schedule(dynamic)
       !$omp do 
       !print*,maxval(abs((-1.)*dtsub*(ap(1:pmax)%fxltot)/(ap(1)%vol*ap(1)%ratiorho))),'mimi1'
       !print*,maxval(abs((ap(1:pmax)%intv-op(1:pmax)%intv)/(ap(1)%vol*ap(1)%ratiorho)/real(iend))),'mimi2'
       !print*,abs(rkcoeffab(rkiter)*dtsub*gaccz*(1.-(1./ap(1)%ratiorho))),'mimi3'
       do p=1,pmax
          if (ap(p)%mslv.gt.0) then
             ap(p)%u = op(p)%u + &
                  (1-colflgx(p))*( &
                  (-1.)*dtsub*(ap(p)%fxltot)/(ap(p)%vol*ap(p)%ratiorho) + &
                  (ap(p)%intu-op(p)%intu)/(ap(p)%vol*ap(p)%ratiorho)/(1.*iend) &
                  ) + &
                  rkcoeffab(rkiter)*dtsub*gaccx*(1.-(1./ap(p)%ratiorho)) + &
                  rkcoeffab(rkiter)*0.5*dtsub*(ap(p)%colfx+op(p)%colfx)/(ap(p)%vol*ap(p)%ratiorho)
             ap(p)%x = op(p)%x + rkcoeffab(rkiter)*0.5*dtsub*(ap(p)%u+op(p)%u)
             ap(p)%v = op(p)%v + &
                  (1-colflgy(p))*( &
                  (-1.)*dtsub*(ap(p)%fyltot)/(ap(p)%vol*ap(p)%ratiorho) + &
                  (ap(p)%intv-op(p)%intv)/(ap(p)%vol*ap(p)%ratiorho)/(1.*iend) &
                  ) + &
                  rkcoeffab(rkiter)*dtsub*gaccy*(1.-(1./ap(p)%ratiorho)) + &
                  rkcoeffab(rkiter)*0.5*dtsub*(ap(p)%colfy+op(p)%colfy)/(ap(p)%vol*ap(p)%ratiorho)
             ap(p)%y = op(p)%y + rkcoeffab(rkiter)*0.5*dtsub*(ap(p)%v+op(p)%v)
             ap(p)%w = op(p)%w + &
                  (1-colflgz(p))*( &
                  (-1.)*dtsub*(ap(p)%fzltot)/(ap(p)%vol*ap(p)%ratiorho) + &
                  (ap(p)%intw-op(p)%intw)/(ap(p)%vol*ap(p)%ratiorho)/(1.*iend) &
                  ) + &
                  rkcoeffab(rkiter)*dtsub*gaccz*(1.-(1./ap(p)%ratiorho)) + &
                  rkcoeffab(rkiter)*0.5*dtsub*(ap(p)%colfz+op(p)%colfz)/(ap(p)%vol*ap(p)%ratiorho)
             ap(p)%z = op(p)%z + rkcoeffab(rkiter)*0.5*dtsub*(ap(p)%w+op(p)%w)

             ap(p)%omx = op(p)%omx + &
                  (-1.)*dtsub*ap(p)%torqxltot/(ap(p)%mominert*ap(p)%ratiorho) + &
                  (ap(p)%intomx-op(p)%intomx)/(ap(p)%mominert*ap(p)%ratiorho)/(1.*iend) + &
                  rkcoeffab(rkiter)*0.5*dtsub*(ap(p)%coltx+op(p)%coltx)/(ap(p)%mominert*ap(p)%ratiorho)
             ap(p)%omy = op(p)%omy + &
                  (-1.)*dtsub*ap(p)%torqyltot/(ap(p)%mominert*ap(p)%ratiorho) + &
                  (ap(p)%intomy-op(p)%intomy)/(ap(p)%mominert*ap(p)%ratiorho)/(1.*iend) + &
                  rkcoeffab(rkiter)*0.5*dtsub*(ap(p)%colty+op(p)%colty)/(ap(p)%mominert*ap(p)%ratiorho)
             ap(p)%omz = op(p)%omz + &
                  (-1.)*dtsub*ap(p)%torqzltot/(ap(p)%mominert*ap(p)%ratiorho) + &
                  (ap(p)%intomz-op(p)%intomz)/(ap(p)%mominert*ap(p)%ratiorho)/(1.*iend) + &
                  rkcoeffab(rkiter)*0.5*dtsub*(ap(p)%coltz+op(p)%coltz)/(ap(p)%mominert*ap(p)%ratiorho)
             ap(p)%phi   = op(p)%phi! + 0.5*rkcoeffab(rkiter)*dtsub*(ap(p)%omz+op(p)%omz)

             ap(p)%omtheta = (ap(p)%omy*cos(ap(p)%phi)) - & 
                  (ap(p)%omx*sin(ap(p)%phi))
             ap(p)%theta = op(p)%theta! + rkcoeffab(rkiter)*0.5*dtsub*(ap(p)%omtheta+op(p)%omtheta)
          endif
       enddo
       !$omp end parallel
       !
       ...
       if ((sumcolrank_all+npmax*Nproc) .ne. 0) then !criterion for occurence of collisions 
          ...
          if ( myid .eq. rankmax.and.iter.eq.1.and.isubstep.eq.iend) then
    ...
    if(isubstep.eq.iend) then
       ...
       do nb=1,8
          ...
          nbrec  = nb
          if (nbsend .gt. 8) nbsend = nbsend-8
          call MPI_SENDRECV(newmaster_nb(1,0),pmax,MPI_INTEGER,neighbor(nbsend),4, &
               newmaster_nb(1,nbrec),pmax_nb(nbrec),MPI_INTEGER,neighbor(nbrec),4, &
               comm_cart,status,error)
       enddo
       !
       do p=1,pmax
          ...
          if (newmaster_nb(p,0).gt.0) then
             ...
             tag = ap(p)%mslv!*10!+nbsend-ap(p)%mslv*10
             ...
             call MPI_ISEND(ap(p)%qmax,send_int,MPI_INTEGER,neighbor(nbsend),tag+1,comm_cart,arrayrequests((nrrequests-1)*2+2),error)
             ap(p)%mslv = -ap(p)%mslv !master is a slave now
             !    print*,tag, 'master2slave',p,-ap(p)%mslv,myid,ap(p)%x,ap(p)%y,ap(p)%z
             !
             ! actually I am cleaning the old master twice: here and some lines of code below
             ! but let's keep it like this for now
             !
             ap(p)%x = 0.
             ap(p)%y = 0.
             ap(p)%z = 0.
             ...
             do nbrec = 1,8
                ...
                do i=1,pmax_nb(nbrec)
                   ...
                   if(newmaster_nb(i,nbrec) .eq. nbrec2.and.idp.eq.idp_nb) then
                      ...
                      nrrequests = nrrequests + 1
                      tag = ap(p)%mslv!*10!+nbrec2-ap(p)%mslv*10
                      !          print*,tag, 'slave2master',p,idp,myid,ap(p)%x
                      call MPI_IRECV(ap(p)%x,send_real,MPI_REAL8,neighbor(nbrec),tag,comm_cart,arrayrequests((nrrequests-1)*2+1),error)
                      call MPI_IRECV(ap(p)%qmax,send_int,MPI_INTEGER,neighbor(nbrec),tag+1,comm_cart,arrayrequests((nrrequests-1)*2+2),error)
                   endif
                enddo
             enddo
          endif
    ...
    !$omp end parallel
       ...
       do idp = 1,np
          ...
          if(found_mstr) then
             ...
             !    enddo
             if(count_slve_loc.gt.0) count_slve = count_slve + 1
          endif
       enddo
       !
...
!       enddo
    ...
    !
    return
  end subroutine intgr_nwtn_eulr_subc
  !
  subroutine sumrk3(pnew,pold)
    implicit none
    integer :: pnew,pold,p
    !
    ! compute particle acceleration and angular acceleration
    !
    rkp(pnew)%dudt   = rkp(pold)%dudt 
    rkp(pnew)%dvdt   = rkp(pold)%dvdt 
    rkp(pnew)%dwdt   = rkp(pold)%dwdt 
    rkp(pnew)%domxdt = rkp(pold)%domxdt 
    rkp(pnew)%domydt = rkp(pold)%domydt 
    rkp(pnew)%domzdt = rkp(pold)%domzdt 
    p = pnew
    if(rkiter.eq.1) then
       rkp(p)%dudt  = ap(p)%u 
       rkp(p)%dvdt  = ap(p)%v
       rkp(p)%dwdt  = ap(p)%w
       rkp(p)%domxdt  = ap(p)%omx 
       rkp(p)%domydt  = ap(p)%omy
       rkp(p)%domzdt  = ap(p)%omz
    elseif(rkiter.eq.3) then
       rkp(p)%dudt   = (ap(p)%u-rkp(p)%dudt)/dt
       rkp(p)%dvdt   = (ap(p)%v-rkp(p)%dvdt)/dt
       rkp(p)%dwdt   = (ap(p)%w-rkp(p)%dwdt)/dt
       rkp(p)%domxdt = (ap(p)%omx-rkp(p)%domxdt)/dt
       rkp(p)%domydt = (ap(p)%omy-rkp(p)%domydt)/dt
       rkp(p)%domzdt = (ap(p)%omz-rkp(p)%domzdt)/dt
    endif
    !!
...
!!$omp do schedule(dynamic)
    ...
    !  if (ap(p)%mslv .gt. 0) then
    !    do l=1,NL
    !      rkp(p)%fxl(l)  = rkp(p)%fxl(l) + ap(p)%fxl(l) ! not multiplied with dVlagr!
    !      rkp(p)%fyl(l)  = rkp(p)%fyl(l) + ap(p)%fyl(l) ! not multiplied with dVlagr!
    !      rkp(p)%fzl(l)  = rkp(p)%fzl(l) + ap(p)%fzl(l) ! not multiplied with dVlagr!
    !    enddo
    ...
    !        (-1.)*ap(p)%torqtheta ! note: not corrected for inertia of fluid inside sphere
...
end module mod_intgr_nwtn_eulr_subc
